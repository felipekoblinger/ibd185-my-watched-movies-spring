...
public class LogbackVerifier implements TestRule {
    ...
    private void expectMessage(Level level, String msg, Class<? extends Throwable> throwableClass) {
    ...
    private void verify() {
        for (final ExpectedLogEvent expectedEvent : expectedEvents) {
            Mockito.verify(appender).doAppend(argThat(expectedEvent::matches));
        }
    }
